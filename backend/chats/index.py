...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if method == 'GET':
        ...
        if chat_id:
            cur.execute(
                "SELECT m.id, m.text, m.sender_id, m.is_read, m.created_at, u.username as sender_username FROM messages m JOIN users u ON m.sender_id = u.id WHERE m.chat_id = %s ORDER BY m.created_at ASC",
                (chat_id,)
            )
